class LoanRepaymentsController < ApplicationController
  ...
  def authorize_resource!(resource = @loan_repayment)
...
def prevent_edit_if_closed
  ...
  private
    ...
    def loan_repayment_params
      params.expect(loan_repayment: [ :user_id, :loan_id, :amount, :repayment_month, :balance_at_time, :balance_after, :bank_charge_paid, :is_refinance_settlement ])
    end
end
